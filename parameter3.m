...
th1x = interp1(time,th1,[0:0.0001:10],'spline');
th2x = interp1(time,th2,[0:0.0001:10],'spline');
th3x = interp1(time,th3,[0:0.0001:10],'spline');
timex = linspace(0,10,100001);
...
N = length(time_torque);
torque_pelvis(N) = torque_pelvis(N-1);
torque_knee(N) = torque_knee(N-1);
torque_ankle(N) = torque_ankle(N-1);
figure;
subplot(3,1,1);
plot(time_torque,torque_pelvis,'g');
xlabel('time [s]');
ylabel('torque [N*cm]');
...
subplot(3,1,3);
plot(time_torque,torque_ankle,'b');
xlabel('time [s]');
ylabel('torque [N*cm]');
